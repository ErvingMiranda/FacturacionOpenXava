...
- Create the action class in the `org.Ezone.POO.Facturacion.actions` package, typically extending `ViewBaseAction`
...
<controller name="YourFirstEntity">
    ...
    <action name="showAmount" mode="detail"
        class="org.Ezone.POO.Facturacion.actions.ShowAmountAction"/>
</controller>
```
...
- ✅ Place action classes under `org.Ezone.POO.Facturacion.actions`
...
### Basic Test Example
```java
package org.Ezone.POO.Facturacion.tests;
...
- Create a `dashboards` package (e.g., `org.Ezone.POO.Facturacion.dashboards`) and place the `Dashboard` class and its auxiliary classes in it.
